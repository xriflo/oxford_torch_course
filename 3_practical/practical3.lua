...
opt.batch_size = 3
opt.train_size = 30000  -- set to 0 or 60000 to use all 60000 training data
opt.test_size = 2000      -- 0 means load all data
...
-- DONE: the code below changes the optimization algorithm used, and its settings
...
local function load_dataset(train_or_test, count)
    ...
    -- DONE: (optional) UNCOMMENT to display a training example
...
for i = 1, iterations do
  ...
  if i % 10 == 0 then
    logProbsTest = model:forward(test.data)
    classProbabilitiesTest = torch.exp(logProbsTest)
    _, classPredictionsTest = torch.max(classProbabilitiesTest, 2)
    ...
    classification_error = 100 * (opt.test_size - predicted_ok) / opt.test_size
  
  end

  if i % 10 == 0 then
    losses[#losses + 1] = minibatch_loss[1] -- append the new loss
    test_losses[#test_losses + 1] = classification_error
  end
...
y = torch.range(1, #test_losses)
gnuplot.plot({'training losses',torch.range(1, #losses),torch.Tensor(losses),'~'},{'test_losses',torch.range(1, #test_losses),torch.Tensor(test_losses),'|'})
--[[
gnuplot.plot({
...
-- DONE: compute test classification error here for the second handin item

print('----------------------')
predicted_ok = 0
for i=1,opt.test_size do
  -- print(classPredictions[i][1]..'vs'..test.labels[i])
